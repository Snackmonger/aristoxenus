...
class Chord(ParserMixin, MaterialMixin, ConverterMixin):
    ...
    about how a chord with an E root (E#, Ebb, etc.) should assume that the 2
    interval must be a D note (D##, Dbb, etc.), and so on. 
    ...
    def from_interval_structure(cls, interval_structure: int, keynote: str = "C") -> "Chord":
        ...
        chord = cls()
        chord.base_chord = keynote + cls.parse_interval_structure_as_chord_symbol(interval_structure)
        chord.notes = interface.render_plain(interval_structure=interval_structure, keynote=keynote)
    ...
    def from_note_names(cls, note_names: Sequence[str]) -> "Chord":
        chord = cls()
        chord.interval_structure = parsing.parse_literal_sequence(note_names=note_names)
        chord.base_chord = note_names[0] + cls.parse_interval_structure_as_chord_symbol(chord.interval_structure)
        chord.intervals = rendering.render_plain(chord.interval_structure, nomenclature.twelve_tone_scale_intervals())
