...
def recognize(
        note_data: str | Sequence[str]
        ) -> list[tuple[int, str, str]]:
    '''
    Recognize a Greek musical symbol, or a group of symbols, 
    and return the corresponding symbol number.
    '''
    symbols: list[str] = []
        ...
        for item in ANCIENT_GREEK_MUSICAL_SYMBOLS:
            if symbol == item['instrumental']:
                identified_symbols.append(
                    (int(item['index']), symbol, 'instrumental'))
            elif symbol == item['vocal']:
                identified_symbols.append(
                    (int(item['index']), symbol, 'vocal'))
...
def parse_formatting(text: str) -> str:
    ...
    for item in ANCIENT_GREEK_MUSICAL_SYMBOLS:
        ...
        text = text.replace(
            '&'+str(item['index'])+'i&', str(item['instrumental']))
