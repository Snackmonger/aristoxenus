...
def render_heptatonic_form(scale_name: str, modal_name: str, keynote: str) -> dict[str, str|tuple[str, ...]|list[str]]:
    ...
    base_scale = {value: key for key, value in intervallic_canon.HEPTATONIC_SYSTEM.items()}[scale_name]
    modal_rotations = keywords.MODAL_NAME_SERIES.index(modal_name)
    for _ in range(modal_rotations):
        base_scale = bitwise.previous_inversion(base_scale, 12)

    render_base = nomenclature.chromatic()
    ...
    chromatic_rendering = tuple(rendering.render_plain(base_scale, render_base))
    forced_rendering = tuple(nomenclature.force_heptatonic(keynote, base_scale))
    best_rendering = tuple(nomenclature.best_heptatonic(keynote, base_scale))
