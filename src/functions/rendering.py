...
def render_plain(
    ...
    ----------
    interval_structure : int
        An integer representing the interval structure to be rendered.
    chromatic_scale : Sequence[str] | None, optional
        The 12-note chromatic scale you want to use to render the structure, 
        by default None, representing the binomials.
...
def render_scientific(
    ...
    ----------
    interval_structure : int
        An integer representing an interval structure of any length and 
        bit-count.
    scientific_range : Optional[Sequence[str]], optional
        An array of scientific note names to use as the basis of the rendering. 
        Must be at least as long as the interval structure, defaults to 12-TET., 
        by default None
