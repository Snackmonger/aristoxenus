...
def parse_slash_chord_symbol(chord_symbol: str) -> int:
    ...
    Examples:
        >>> bin(parse_slash_chord_symbol("Gmaj/B"))
        '0b100001001'
        >>> bin(parse_slash_chord_symbol("Fmin7b5/Eb"))
        '0b100100101'
        >>> bin(parse_slash_chord_symbol("Ab7b5/A"))
        '0b101000101001'
    '''
    if chord_symbol.count('/') != 1:
        raise errors.ChordSymbolError(chord_symbol)
    main, bass = chord_symbol.split("/")
    chord_structure = parse_chord_symbol(main)
    ...
    return parse_literal_sequence(chord_note_names)
...
def parse_polychord_symbol(chord_symbol: str) -> int:
    ...
    Examples
        >>> chord = 'Cmaj7@Ebm7b5'
        >>> x = parse_polychord_symbol(chord)
        >>> bin(x)
        '0b10101011011001'
...
def parse_heptatonic_scale_structure(interval_structure: int) -> tuple[str, str]:
    ...
    for heptatonic_supertype in intervals.HEPTATONIC_ORDER:
        ...
        if interval_structure in supertype_modes:
            parent = intervals.HEPTATONIC_SYSTEM_BY_NUMBER[heptatonic_supertype]
