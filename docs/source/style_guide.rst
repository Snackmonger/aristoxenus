...
Add/No Notation
+++++++++++++++
The chord symbols generated by Aristoxenus will only use 'no3' and 'no5'; other missing intervals will be re-spelled as different chords.

The chord symbols generated by Aristoxenus will use 'add' whenever a chord has an extra 2, 4, or 6, beyond a recognizable base structure. If
a chord uses a 2 or 4 but not a 3 or b3, then the chord will be characterized as 'sus'. A chord with a 6 will form a secondary suffix (e.g. "maj6"),
unless it already has a primary suffix (i.e. a 7), e.g. "maj7add6". When a chord has an extension, it will be characterized as an 'add' if it 
is not altered compared to the major scale interval. Thus, a #11 will simply be suffixed after the previous suffix (e.g. "C7#11"), but a natural 11
will be characterized as 'add' (e.g. "Amin7add11"). This distinction helps make chords more readable by preventing numerals from colliding.

The chord symbols parsed by Aristoxenus will attempt to incorporate all user-generated 'add' and 'drop' symbols, even when they are counter-intuitive.
These symbols are always parsed last, so they can be used to make explicit corrections to a chord's structure.

The 'add' type suffix indicates that the specified interval is to be added to the structure of the preceding symbol, without implying
any other interval or structure.

    - Cmaj7add13 -> C, E, G, B, A

The 'no' type suffix indicates that the specified interval is removed from the structure of the preceding symbol, without implying the removal of
any other interval or structure. 
...
The symbol interpretation always assumes that a chord contains a p5 unless one of the other symbols indicates otherwise.
Therefore, we refer to augmented chords as 'maj#5' and diminished chords as 'mb5'. If either of these symbols appears in
the compound symbol and is not qualified by "add" or "no", then it will be understood as replacing the implicit p5 in the chord.
...
In situations where a chord has both a seventh and a sixth, the sixth will be characterized as 'add'. Contrary to other
extensions, an altered sixth will always be characterized as 'add' and never directly suffixed to a 7 suffix.

Examples:

- "C7add6" -> C E G A Bb
- "Cmaj7add#6" -> C E G A# B
