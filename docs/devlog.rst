...
The main widget displays a segment of a fretboard, with different coloured shapes to represent the notes in the scale/arpeggio. The shapes can be controlled by the user, so
that different intervals are displayed with different shapes and colours. Each shape can be set to display the note name, interval name, or finger. The fingering of individual
strings is controlled by a button, rather than trying to get the computer to decide which fingerings are appropriate. Ultimately, this approach is better for the user; rather than
the diagram tool trying to show how the user **should** finger a particular form, we allow the user to play around with various possibilities until they find something that suits their 
needs. In its current form, it works pretty well for standard tuning; if I've written everything right, we should be able to change the basic fretboard (number of strings & tuning) so
that the tool can display alternate guitar layouts.

I have written a hack-y version of the widget, and I am now turning my attention to separating responsibilities into MVC pattern classes and recreating the widget in a slightly more 
legible form. This kind of tool benefits from updating its display for each state change, so I'm not entirely sure that a django app will suit its needs. I don't know anything about
javascript yet, but I suspect that it's probably the best tool for a web app version of the program. Another thing for my to-do list.
